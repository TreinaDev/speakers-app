    ...
    begin
      ...
      if response.success?
        ...
        result = schedule_items['schedule_items'].map do |item|
          ScheduleItem.new(id: item['id'], title: item['title'], description: item['description'], speaker_email: item['speaker_email'], length: item['length'], start_time: item['start_time'])
        end
      end
